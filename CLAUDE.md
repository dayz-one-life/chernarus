...
- `docs/tools/` — reusable maintenance scripts, kept out of the FTP deploy. `road-to-badlands/nerf_loot.py` re-derives the loot nerf (halve `nominal`/`min`, round up, excluding `deloot="1"` and `ContaminatedArea` types) on top of a fresh upstream mission drop; it preserves the source file's line endings (`newline=""`) and aborts loudly if the element count changes. The former `buff_zeds.py` was retired when the zed buff was reverted to vanilla — do not re-apply a zed buff on upstream merges. `env/zombie_territories.xml` and `cfggameplay.json` ambient temps are now pure vanilla.
